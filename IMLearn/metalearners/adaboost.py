...
class AdaBoost(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        self.models_ = [BaseEstimator] * self.iterations_
        self.D_ = np.ones(y.shape[0]) / y.shape[0]
        for i in range(self.iterations_):
            self.models_[i] = self.wl_().fit(X, y * self.D_)
            y_predict = self.models_[i].predict(X)
            eps = np.sum(self.D_[y != y_predict])
            self.weights_[i] = np.log(1.0 / eps - 1) * 0.5
            self.D_ *= np.exp(-y * self.weights_[i] * y_predict)
    ...
    def partial_predict(self, X: np.ndarray, T: int) -> np.ndarray:
        ...
        for i in range(T):
            y_predict += (self.models_[i].predict(X) * self.weights_[i])
